...
if(isset($_POST['inedu'])){
    ...
    $select_query="SELECT `eduid`, `eduname`, `edudate`, `eduplace`, `edug` FROM `education` WHERE eduname='$eduname'";
    $result_select=mysqli_query($con,$select_query);
    $number=mysqli_num_rows($result_select);
    if($number>0){
        echo "<script>alert('Education already exists.')</script>";
    }else{
        $insert_query="INSERT INTO `education`(`eduname`, `edudate`, `eduplace`, `edug`) VALUES ('$eduname','$edudate','$eduplace', '$edug')";
    ...
    <form action="" method="POST">
      <div class="invisible">
        ...
        <div class="mb-3">
          ...
          <input class="form-control" type="text" placeholder="Course/Strand" aria-label="default input example" name="edun">
        </div>
        <div class="mb-3">
          <label for="exampleInputEmail1" class="form-label text-dark fw-bold fs-3 font-monospace">Date</label>
          <input class="form-control" type="date" aria-label="default input example" placeholder="" name="edud">
        </div>
        <div class="mb-3">
          <label for="exampleInputEmail1" class="form-label text-dark fw-bold fs-3 font-monospace">Place</label>
          <input class="form-control" type="text" placeholder="School address" aria-label="default input example" name="edup">
        </div>
        <div class="mb-3">
          <label for="exampleInputEmail1" class="form-label text-dark fw-bold fs-3 font-monospace">Grade</label>
          <input class="form-control" type="text" placeholder="School grade" aria-label="default input example" name="edug">
        </div>
